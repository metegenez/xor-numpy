...
class NeuralNetwork:
    ...
    def predict(self, x):
        ...
        for i in range(0, len(self.weights)):
            val = self.activation_func(numpy.dot(val, self.weights[i]))  # burak hata
    ...
    def plot_decision_regions(self, X, y, points=200):
        ...
        # x1_min, x1_max = -10, 11 # x2_min, x2_max = -10, 11
        resolution = max(x1_max - x1_min, x2_max - x2_min) / float(points)
        xx1, xx2 = numpy.meshgrid(numpy.arange(x1_min, x1_max,
                                               resolution), numpy.arange(x2_min, x2_max,
                                                                         resolution))
